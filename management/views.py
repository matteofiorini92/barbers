...
def new_treatment(request):
    if not request.user.is_superuser:
        messages.error(request, "Sorry, you can't access this page")
        ...
        if request.method == 'POST':
            ...
            form = TreatmentForm(
                initial={'price': '0.00', 'duration': 'hh:mm:ss'})
...
def edit_treatment(request):
    if not request.user.is_superuser:
        ...
        if request.method == 'POST':
            ...
            form = TreatmentForm(
                request.POST, request.FILES, instance=treatment)
            if form.is_valid():
                messages.success(
                    request,
